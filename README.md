...
On first launch, the app extracts the bundled pier and starts the Urbit runtime automatically. A dashboard shows your ship's status, recent logs, and controls to stop, restart, or retry booting if something goes wrong.

Once the ship is ready, click **Open Ship** to open your ship's web interface in a browser. You can find your access code from the diagnostics panel.
...
|---|---|
| `SHIP_LAUNCHER_VERE_PATH` | Path to a `vere` binary (defaults to bundled binary in app resources) |
...
To bundle a real ship, place the vere binary, pier archive, and manifest in the Tauri resources directory and configure `tauri.conf.json` accordingly.
